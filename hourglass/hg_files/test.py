...
import hourglass.hg_files.img as img_ut
import hourglass.hg_files.ref as ds
...
def post_process(det, mat_, trainval, c=None, s=None, resolution=None):
    ...
    if len(cropped_preds) > 0:
        cropped_preds[:,:,:2] = img_ut.kpt_affine(cropped_preds[:,:,:2] * 4, mat) #size 1x16x3
        
    preds = np.copy(cropped_preds)
    ##for inverting predictions from input res on cropped to original image
    if trainval != 'cropped':
        for j in range(preds.shape[1]):
            preds[0,j,:2] = img_ut.transform(preds[0,j,:2], c, s, resolution, invert=1)
    return preds
...
def inference(img, func, config, c, s):
    ...
    mat_ = img_ut.get_transform(center, scale, res)[:2]
    inp = img/255
...
def get_img(config, num_eval=2958, num_train=300):
    ...
    for i in tr:
        ...
        c = train_f['center'][i]
        s = train_f['scale'][i]
        im = img_ut.crop(orig_img, c, s, (input_res, input_res))
